...
\begin{itemize}
	...
	\item [·]存在负向量$对于a=(a_1,...,a_n)，存在负向量-a=(-a_1,...,-a_n)，满足a+(-a)=\mathit{O}$
...
对于$W_1,W_2\subseteq V$,$W_1\cap W_2=W_3,W_1+W_2=W_4。W_3和W_4$仍是V的子空间，但是$W_1\cup W_2=W_5,W_5$不一定是子空间。\\
\indent
...
\subsubsection{逆}
对于一个矩阵$A\in F^{n\times n}$，若存在矩阵$B\in F^{n\times n},使得AB=BA=I(I为单位方阵)$，则称A为可逆矩阵，B为A的逆矩阵且记为$A^{-1}$。\\
\indent
方阵可逆，当且仅当其行列式不为零。\\
\indent
...
\indent
若方阵$A\in F^{n\times n}可以经过有限次初等变换变成方阵B\in F^{n\times n}$，则称方阵A，B相抵，也称等价。若两个方阵相抵，则$Rank(A)=Rank(B)$
\subsection{线性映射}
若一个映射满足$f(\alpha x+\beta y)=\alpha f(x)+\beta f(y)$，则称该映射为线性映射。线性映射$\sigma：X\rightarrow AX$可以通过用矩阵A左乘自变量X实现。若$\sigma是F^{n\times 1}到自身的映射，则称为线性变换(线性自同态映射)$。\\
\indent
对于线性映射$\sigma U\rightarrow V，对于两个基，有X\rightarrow AX，Y\rightarrow BY，则A与B相抵。$\\
\indent
若该映射为线性变换，则A与B相似。即同个线性变换在不同基下的矩阵相似。
\subsection{相似}
$设A,B是可逆方阵，如果存在n阶可逆方阵P，使得B=P^{-1}AP，则称A与B相似。$
\subsubsection{性质}
\begin{itemize}
	...
	\item [-]$对于任意多项式f(x)，有f(PAP^{-1})=Pf(A)P^{-1}$（实质是上一条的一般情况）
	\item [-]属于同一个方阵的不同特征值对应的特征向量线性无关。
\end{itemize}
\subsubsection{相似对角化条件（具体可参照维基百科“可对角化矩阵词条”）}
先阐述两个概念：代数重数即$\lambda$作为方阵$A$的特征多项式的根的次数，几何重数即特征值相对应的特征空间（即$\lambda I -A的核$）的维数。
\begin{itemize}
...
\indent
同时，如果$p=n$，则$\Lambda与S正定$，如果$p=0$，则$\Lambda与S半负定$。
\subsubsection{顺序主子式}
